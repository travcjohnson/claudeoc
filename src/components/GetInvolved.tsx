...
export function GetInvolved() {
  return (
    <section id="get-involved" className="bg-ivory-medium py-24 dark:bg-stone-900 lg:py-32">
      <div className="max-w-[89.5rem] mx-auto px-6 lg:px-10">
        ...
        <div className="space-y-12">
          ...
          <div className="bg-white dark:bg-stone-800 rounded-xl border border-slate-dark/10 dark:border-white/10 p-8">
            <div className="mb-6">
              <h3 className="font-sans font-semibold text-2xl text-slate-dark dark:text-cream mb-2">
                Lead a Conversation
              </h3>
              <p className="font-serif text-slate-light dark:text-muted leading-relaxed max-w-2xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-8">
              {topicCards.map((card) =>
                card.custom ? (
                  <div
                    key={card.label}
                    className="rounded-lg border border-dashed border-clay p-4 flex items-center gap-3 bg-white dark:bg-stone-900"
                  >
                  ...
                  <div
                    key={card.label}
                    className="rounded-lg border border-slate-dark/10 dark:border-white/10 p-4 flex items-center gap-3 bg-white dark:bg-stone-900"
                  >
                    <span className="text-lg leading-none">{card.icon}</span>
                    <span className="font-sans font-medium text-sm text-slate-dark dark:text-cream">
                      {card.label}
                    </span>
                  </div>
                )
            ...
            <a
              ...
              className="inline-flex items-center font-sans font-medium text-slate-dark dark:text-cream hover:text-clay dark:hover:text-clay transition-colors"
            >
          ...
          {/* --- 2. For Developers --- */}
          <div className="bg-oat/50 dark:bg-stone-800 rounded-xl p-8">
            <h3 className="font-sans font-semibold text-2xl text-slate-dark dark:text-cream mb-3">
              Builders &amp; Developers
            </h3>
            <p className="font-serif text-slate-light dark:text-muted leading-relaxed max-w-2xl mb-6">
            ...
            <ul className="space-y-3 mb-6">
              {developerPoints.map((point) => (
                <li key={point} className="flex items-start gap-3">
                  ...
                  <span className="font-serif text-slate-dark dark:text-cream">{point}</span>
                </li>
              ))}
            </ul>

            <p className="font-serif text-slate-light dark:text-muted text-sm mb-6 max-w-xl">
            ...
            <a
              href="#connect"
              className="inline-flex items-center bg-slate-dark text-ivory-light rounded-lg px-6 py-3 font-sans font-medium hover:bg-slate-medium transition-colors dark:bg-clay dark:hover:bg-accent"
            >
