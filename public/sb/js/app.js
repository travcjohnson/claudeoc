...
(function () {
  ...
  function currentSchedule() {
    const el = document.querySelector('.slide[data-slide="' + Slides.current() + '"]');
    const key = el && el.getAttribute('data-section');
    if (!key) return null;
    for (let i = 0; i < SCHEDULE.length; i++) {
      if (SCHEDULE[i].key === key) return SCHEDULE[i];
    }
    return null;
  }

  function renderTimer() {
    const el = document.getElementById('deck-timer');
    if (!el) return;
    const now = new Date();
    const clockEl = el.querySelector('.deck-timer__clock');
    if (clockEl) clockEl.textContent = formatClock(now);

    const sectionEl = el.querySelector('.deck-timer__section');
    const progEl = el.querySelector('.deck-timer__progress');
    const barEl = el.querySelector('.deck-timer__bar');
  ...
  function toggleTimer() {
    ...
    document.body.classList.toggle('timer-hidden', !timerVisible);
  }

  function initTimer() {
    if (document.getElementById('deck-timer')) return;
    ...
    document.addEventListener('slidechange', renderTimer);
  }
  ...
  function onKeyDown(e) {
    ...
    if (k === 't' || k === 'T') { toggleTimer(); e.preventDefault(); return; }
